...
func determineMove(oppPlayed string, finish string) int {
	if finish == "Y" { // draw
		us := toRPS(oppPlayed)
		them := toRPS(oppPlayed)
		return score(us, them)
	} else if finish == "X" { // loss
		var us string
		if oppPlayed == "A" { // rock
			us = toRPS("Z") // scissors
		} else if oppPlayed == "B" { // paper
			us = toRPS("X") // rock
		} else { // scissors
			us = toRPS("Y") // paper
		}
...
func main() {
	...
	for _, element := range data {
		...
		totalScore += determineMove(them, us)
	}
